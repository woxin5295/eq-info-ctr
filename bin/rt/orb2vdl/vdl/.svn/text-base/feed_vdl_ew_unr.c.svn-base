 ...
 *   feed_vdl_ew.c
	...
	more work than justified at this time. The OS2 references are historic, but
	maybe some day...
 */
#ifdef _OS2
...
#define   MAXLOGO   10
MSG_LOGO  GetLogo[MAXLOGO];     /* array for requesting module,type,class */
short 	  nLogo;
...
/*	fprintf(logout,"time= %4d %3d-%2d:%2d:%2d.%3d %d/%d/%d ich=%d tm=%d\n",
    ...
    ******************/
 	memcpy(ia, (long *)( outMsg+sizeof(TRACE_HEADER)), 		
		((int)((TRACE_HEADER*)outMsg)->nsamp)*sizeof(long));/* move to VDL buffer*/
	...
	/* debug if(res==GET_OK)logit("et","Got message from transport of %ld bytes, res=%d\n",recsize,res);*/
      ...
      **********************************/
      if( res != GET_OK )
         ...
         if( res==GET_TOOBIG ) 
            {
            sprintf( Text, "msg[%ld] c%d m%d t%d too long for target",recsize, (int) reclogo.class,
            ...
            {
            sprintf( Text, "missed msg(s) c%d m%d t%d in %s",(int) reclogo.class,
			    (int) reclogo.mod, (int)reclogo.type, RingName );
            vdl_ew_status( TypeError, ERR_MISSMSG, Text );
            }
         else if( res==GET_NOTRACK ) 
           {
            sprintf( Text, "no tracking for logo c%d m%d t%d in %s",(int) reclogo.class,
			   (int) reclogo.mod, (int)reclogo.type, RingName );
 ...
 ***************************/
  ...
  /*6*/     else if( k_its("GetMsgLogo") ) {
                ...
                nLogo++;
                init[6] = 1;
            /*    printf("GetLogo[%d] class:%d module:%d type:%d\n",
		        nLogo, (int) GetLogo[nLogo].class,
