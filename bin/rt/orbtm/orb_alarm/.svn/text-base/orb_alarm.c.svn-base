 ...
 *   Copyright (c) 2007 Nikolaus Horn /  Vienna, Austria
 *   All rights reserved. 
 *                                                                     
 *   Written by Dr. Kent Lindquist, Lindquist Consulting, Inc. 
 *   Modified by Nikolaus Horn / Vienna, Austria
 *
...
char	*Pfname = "orb_alarm";
Pf	*pf;

void
usage()
{
    	cbanner( "$Date: 2011-05-12 19:02:10 $", 
	      "[-p pfname] [-m match] [-r reject] [-S statefile] "
	      "[-n maxpkts] [-d dbname] [-v] [-V] "
	      "orbname [start-time [period|end-time]]\n",
	      "Kent Lindquist / Nikolaus Horn", 
	      "Lindquist Consulting, Inc. / Vienna, Austria", 
	      "kent@lindquistconsulting.com / Nikolaus.Horn@zamg.ac.at" ) ;
...
add_nearest( void *vstack, Dbptr db, char *function_string )
{
	...
	(void) sprintf( expr, "azimuth(lat,lon,%f,%f)", lat, lon );
	dbex_compile( dbplaces, expr, &az_expr, 0 );

	(void) sprintf( expr, "distance(lat,lon,%f,%f)*111.195", lat, lon );
	...
	compass = windrose( azimuth );

	dbgetv( dbplaces, 0, "place", place, 0 );

	(void) sprintf( expr, "%.0f km %3s of %s", 
...
rtmail_send( char *subject, char *body, char *to )
{
	...
	(void) sprintf( cmd, "rtmail -s '%s' %s", subject, to );
...
register_alarm( Alarm *alarm )
{
	...
	(void) sprintf( alarmid, "%ld", alarm->alarmid );
...
verify_unique( char *alarmname, char *alarmkey )
{
	...
	(void) sprintf( key, "%s:%s", alarmname, alarmkey );
...
void process_threshold( Dbptr db ) {
	...
	char	alarmkey[STRSZ];
	char	alarmid[STRSZ];
	char	*alarmclass = "threshold";
	Arr	*message_expressions_in = 0;
	char	literal[STRSZ];
	char	*notification_string;
	int		notify_all=0;
	int		notify_off=0;
	...
	int	irecipient;
	int	rc;
	char sta[STRSZ],chan[STRSZ],alarm_state[STRSZ];
	double time,alarm_time;
	char *ts,*ss;
	...
	rc = pfresolve( pf, "alarms{threshold}", 0, &pfcandidates );
	...
	for( ialarmname = 0; ialarmname < maxtbl( alarmnames ); ialarmname++ ) {
		...
		notification_string=pfget_string( pfcandidate, "notifications" );

		if (notification_string != (char *) NULL) {
			if (strmatches(notification_string,"all|ALL|All",0)){
				notify_all=1;
				notify_off=0;
			} else if (strmatches(notification_string,"on-off|ON-OFF|On-Off",0)){
				notify_off=1;
				notify_all=0;
			} else {
				notify_off=0;
		...
		if( ! triggered ) {
			...
			if( VeryVerbose ) {
				...
				elog_notify( 0, 
					...
					"threshold matches trigger_condition: "
					"<< %s >>\n",
					get_threadname(),
					alarmname, 
					trigger_condition_template );
		...
		dbgetv( db, 0, "sta", sta, 
			       "chan", chan, "time", &time, "alarm_time", &alarm_time, "alarm_state", &alarm_state, 0 );

		ts=epoch2str(time, "%Y%j%H%M%S");
		ss=epoch2str(alarm_time, "%M%S");

		/* this influencesa how many alarms are sent. Currently only ONE
		   email per alarm, regardless of later changes in alarm-level etc...*/

		if (notify_all) {
			(void) sprintf( alarmkey, "%s_%s:%s@%s", sta,chan,ss,ts );
		} else if (notify_off && strmatches(alarm_state,"OFF|off",0)) {
			(void) sprintf( alarmkey, "%s_%s:%s@%s", sta,chan,ss,ts );
		} else {
			(void) sprintf( alarmkey, "%s_%s@%s", sta,chan,ts );
		}
		...
		/* TM have no evid or orid, modified by mitch Wed Jun 25 09:54:02 AKDT 2008 
		* alarm->evid = evid;
		* alarm->orid = orid;
		*/
		alarm->evid = -1;
		...
		(void) sprintf( literal, "\"%s\"", alarmid );
		setarr( alarm->message_expressions, 
				"alarmid", 
				strdup( literal ) );

		(void) sprintf( literal, "\"%s\"", alarmkey );
		setarr( alarm->message_expressions, 
				"alarmkey", 
				strdup( literal ) );

		(void) sprintf( literal, "\"%s\"", alarmname );
		setarr( alarm->message_expressions, 
				"alarmname", 
				strdup( literal ) );

		(void) sprintf( literal, "\"%s\"", alarmclass );
		setarr( alarm->message_expressions, 
				"alarmclass", 
				strdup( literal ) );
...
process_origin( Dbptr db ) 
{
	...
	for( ialarmname = 0; ialarmname < maxtbl( alarmnames ); ialarmname++ ) {
		...
		/* Mitch fixed bug with triggered variable, changed from int to long*/
		rc = dbex_evalstr( db, trigger_condition, dbBOOLEAN, &triggered );
		free( trigger_condition ); 
		...
		(void) sprintf( alarmkey, "evid%ld", evid );
...
process_db( Dbptr db ) 
{
	...
	} else if ( ! strcmp( table_name, "tm" ) ) {

		printf("calling process_threshold\n");
		process_threshold( db );
...
database_watch( void *arg )
{
	...
	while( true ) {
		...
		for( ialarm = 0; ialarm < nalarms; ialarm++ ) {
			...
			(void) sprintf( expr, "alarmname == \"%s\" && alarmkey == \"%s\"",
					alarm->alarmname, 
					alarm->alarmkey );
			...
			if( db.record >= 0 ) {
				...
				if( strcmp( ackauth, "-" ) == 0 ) {
					...
					(void) sprintf( ackauth, "\"%s\"", tempstr );
	...
	while( alarm_count() > 0 || ! Done ) {
		...
		for( ikey = 0; ikey < maxtbl( keys ); ikey++ ) {
			...
			if( ! alarm->wait_ack ) {
				...
				(void) sprintf( expr, "maximum time of %s to wait for "
...
orb_watch( void *arg )
{
	...
	register_threadname( thr_self(), "orb_watch" );
...
fill_regions() 
{
	...
	for( dbregions.record = 0;
	       (long) dbregions.record < (long) nrecs; 
		 dbregions.record++ ) {

		dbgetv( dbregions, 0, "regname", &region, 0 );

		(void) sprintf( expr, "dbsubset regname == \"%s\"", region );
...
main( int argc, char **argv )
{
	...
	elog_notify(0, 
		"[%s]: %s $Revision: 1.5 $ $Date: 2011-05-12 19:02:10 $\n",
		get_threadname(),
		Program_Name);
	...
	rc = thr_create( NULL, 0, orb_watch, 0, 0, &orb_watch_tid );
