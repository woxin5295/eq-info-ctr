...

{
	...
	if (qpl == NULL) {
		qpls = (QPlot *) my_malloc ("qpbin: qpls", sizeof(QPlot));
		if (qpls == NULL) {
		...
		qpls->noverlaps = 0;
		qpls->overlaps = NULL;
		qpls->min = (float *) my_malloc ("qpbin: qpls->min", npixels*sizeof(float));
		if (qpls->min == NULL) {
			fprintf (stderr, "qpbin: Malloc error.\n");
			qpfree (qpls);
			return (NULL);
		}
		qpls->max = (float *) my_malloc ("qpbin: qpls->max", npixels*sizeof(float));
		if (qpls->max == NULL) {
			fprintf (stderr, "qpbin: Malloc error.\n");
			qpfree (qpls);
			return (NULL);
		}
		qpls->in = (float *) my_malloc ("qpbin: qpls->in", npixels*sizeof(float));
		if (qpls->in == NULL) {
			fprintf (stderr, "qpbin: Malloc error.\n");
			qpfree (qpls);
			return (NULL);
		}
		qpls->out = (float *) my_malloc ("qpbin: qpls->out", npixels*sizeof(float));
		if (qpls->out == NULL) {
			fprintf (stderr, "qpbin: Malloc error.\n");
			qpfree (qpls);
			return (NULL);
		}
		qpls->numb = (int *) my_malloc ("qpbin: qpls->numb", npixels*sizeof(int));
		if (qpls->numb == NULL) {
			fprintf (stderr, "qpbin: Malloc error.\n");
			qpfree (qpls);
			return (NULL);
	...
	if (np > 1) {
		if (qpls->overlaps == NULL) {
			qpls->overlaps = (QPlot *) my_malloc ("qpbin: qpls->overlaps", sizeof(QPlot));
			if (qpls->overlaps == NULL) {
				fprintf (stderr, "qpbin: Malloc error.\n");
				return (NULL);
			}
			...
			(qpls->noverlaps)++;
			qpls->overlaps = (QPlot *) my_realloc ("qpbin: qpls->overlaps", qpls->overlaps, qpls->noverlaps*sizeof(QPlot));
			if (qpls->overlaps == NULL) {
		...
		qov->np = 0;
		qov->istart = ist;
		qov->min = (float *) my_malloc ("qpbin: qov->min", npix*sizeof(float));
		qov->max = (float *) my_malloc ("qpbin: qov->max", npix*sizeof(float));
		qov->in = (float *) my_malloc ("qpbin: qov->in", npix*sizeof(float));
		qov->out = (float *) my_malloc ("qpbin: qov->out", npix*sizeof(float));
		qov->numb = (int *) my_malloc ("qpbin: qov->numb", npix*sizeof(int));
...

{
	if (!qpl) return;
	qpfreeoverlaps (qpl);
	if (qpl->min) my_free (qpl->min);
	if (qpl->max) my_free (qpl->max);
	if (qpl->in) my_free (qpl->in);
	if (qpl->out) my_free (qpl->out);
	if (qpl->numb) my_free (qpl->numb);
	my_free (qpl);
}
...

{
	...
	for (i=0; i<qpl->noverlaps; i++) {
		if (qpl->overlaps[i].min) my_free (qpl->overlaps[i].min);
		if (qpl->overlaps[i].max) my_free (qpl->overlaps[i].max);
		if (qpl->overlaps[i].in) my_free (qpl->overlaps[i].in);
		if (qpl->overlaps[i].out) my_free (qpl->overlaps[i].out);
		if (qpl->overlaps[i].numb) my_free (qpl->overlaps[i].numb);
	}
	my_free (qpl->overlaps);
	qpl->overlaps = NULL;
	qpl->noverlaps = 0;
}
...
/* $Id: qplot.c,v 1.1 2001-06-15 00:18:31 kent Exp $ */
