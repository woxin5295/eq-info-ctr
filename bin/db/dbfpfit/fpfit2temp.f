...
c
         print *, '1st coarse -180 to -10 by ', secinc
...
9872    call search2 (bot2, botmn, botmx, chart, coef, fitmx, fitmn, 
     ...
     *  uniq, wtobs)
        print *, '    fitmn,    fitmx,   smnlow,    smnhi,   ',
     *  'smxlow,     smxhi '
        print '(6f10.2)',  fitmn, fitmx, smnlow(ic), smnhi(ic), 
     *  smxlow(ic), smxhi(ic)
c
c
...
c
	...
	print *, 'dippb(ic) = ', dippb(ic)
        rakkb(ic) = rkksv(mmx, ic) 
        fitb(ic) = fitmx
        botb(ic) = botmx
        write(33, '(i5, 2(3f7.1, 2g10.4, a))') ic, dpdirb(ic), 
     1  dippb(ic), rakkb(ic), fitb(ic), botb(ic), chartb(ic)(1:14),
     1                                             dpdirs(ic), 
     1  dipps(ic), rakks(ic), fits(ic), bots(ic), charts(ic)(1:14)
        if(fitmx .gt. fitmxx) then
          print *, 'current max = ', fitmx, ' mmx, ic = ', mmx, ic
          print *, 'dip dir    dip    rake = ', dpdirb(ic), dippb(ic),
     *    rakkb(ic)
          fitmxx = fitmx
          icmx = ic
          botmxx = botmx
...
c
      if(1. - fitmxx .lt. fitmnn) then
        print *, 'biggest fit wins, with ic = ', icmx
        print *, 'in this case best solution is flipped'
        botmax = botmxx
        fitmin = 1. - fitmxx
        ...
        if (islip1 .lt. -180) islip1 = islip1 + 360
         ...
         print *, 'smallest wins, with ic = ', icmn
        botmax = botmnn
        fitmin = fitmnn
...
c       they are the same, so consider the bot value
      ...
      if (lstall) then
        print *, 'coarse grid search solution dip dir, dip, rake, fit:'
...
c
      print *, 'do fine search for the best solution'
      print *, 'nphif, dphif, ndelf, ddelf, nlamf, dlamf'
      print *,  nphif, dphif, ndelf, ddelf, nlamf, dlamf
c
...
c     real              slip2           ! (output) fault slip angle in degrees
      print *, 'input to fsearch2: idpdr1, idip1, islip1, nr'   
      print *, idpdr1, idip1, islip1, nr
c
...
c angle. 
c
      print *, 'express solution in terms of dip etc'
      idpdr2 = iround(dpdr2)
      idip2 = iround(dip2)
      print *, 'idip2 = ', idip2
      islip2 = iround(slip2)
...
c     if(fitlim .gt. .5) fitlim = .5
      print *, 'fitmin, fitlim ', fitmin, fitlim
...
ccc       if ( (fits(ic) .gt. fitlim) .and. (fits(ic) .lt. .5) ) then
          if (fits(ic) .lt. .5) then
            print *, 'try to improve #', ic, ' fitmin = ', fits(ic)
c
...
c
            ...
            dipps(ic) = dipp(mmn)
            rakks(ic) = rakk(mmn)
            if ( fitmn .le. fitlim) then
              print *, 'fitmn improved from ', fits(ic), ' to ', fitmn
            endif
            fits(ic) = fitmn
          endif
c
...
ccc  *         ((1. - fitb(ic)) .lt. .5) ) then
            print *, 'try to improve #', ic, ' fitmx = ', fitb(ic)
c           first define the set of nsec+1 tm's
            print *, 'find maximum in ', smxlow(ic), 
     *      ' to ', smxhi(ic), ' by 1.'
...
c
            ...
            dippb(ic) = dipp(mmx)
            rakkb(ic) = rakk(mmx)
            if ( (1. - fitmx) .le. fitlim) then
              print *, 'fitmx improved from ', fitb(ic), ' to ', fitmx
            endif
            fitb(ic) = fitmx
          endif
c
...
45          continue
           ...
           if (ic .eq. m1) then 
             m1s = nstar
             print *, 'the index of the best solution is m1s = ', m1s
           endif
        endif
...
48    continue
      print *, 'found ', nstar, ' good solutions'
c
c
...
c assign quality code to solution
       print *, 'assign quality code to solution'
c
            ftqual(2:2) = '|'
...
c
            print *, 'accumulate statistics on fit parameter dist.'
