...
c                    modified 3/20/87  j. c. lahr
      ...
      logical    	darkflt    ! true if first nodal plane is to be plotted darker
...
c                                    d=event date
      ...
      integer           getenv_s   ! returns value of environmental variable
...
c                                    3  christensen
      character*1       ins        ! n or s
      character*1       iew        ! e or w 
...
c                                  ! paper rotation:-pi/2 --> -x up
      character*132     ps_file    ! post script file from environment variable
      ...
      integer           status     ! status of environmental variable
...
c begin code
      ...
      tarea = 0.
c
      print *, 'Welcome to fppage!  this is the alaska version'
      print *, 'of fppage.'
      print *, ' '
...
10    filnam = aask('Name of input .ray, .extsum, or christensen file',
     * 'fault.extsum', -50)
      ...
      if(inftyp .eq. 1) then
        ...
        pltpol = torf('Do you want to plot first motion data', .true.)
        if (pltpol) then
          psym = 'P'          
        end if
        ...
        fmt3 = aask('Format', '(a80, 1x, f3.0, f3.0, f4.0)', -50)
      endif

      print *, ' Do you want the first nodal plane to be plotted'
      darkflt = torf(' with a thicker line', .false.)

11    print *, ' Name of output file with computed parameters'
      filnamo = aask(' for each event', 'fppage.out', -50)
      call openit('write', filnamo, 10, ierr)
      if(ierr .ne. 0) go to 11

12    status = getenv_s ('PS_FILE', ps_file)
      if(status .ne. 1) then
	ps_file = 'fppage.ps'
      endif
	...
	stop
      ...
      write(10, '(a)')' Table 1. Hypocenter and focal mechanism paramete
     1rs'  
      write(10, '(2a,/)') ' fppage read focal parameters from:  ', 
...
400   format(1h ,'Event   Date      Time      Latitude  Longitude ',
     ...
     *          '  no.  yr mo da hr mn sec    deg  min  deg   min ',
...
50    print *, ' Name of file with format and character'    
      filfmt = aask(
     *  ' parameters', '/usr/local/aeic/5.2-64/data/fpfit_lahr/fppage.ctl',
     *  -50)
...
c check page format
      ...
      answer = torf('Do you wish to continue',
     *  .true.)
...
c first and last, smaller moves down
c second and third, smaller moves to the left
c        call symbol (x - sin(prot)*ussiz/2.2 - cos(prot)*ussiz/2.8,
c    *   y + sin(prot)*ussiz/2.8 - cos(prot)*ussiz/2.2,
c    *   ussiz, 'P', -prot/rad, 1)
...
815       format (3x, f3.0, f3.0, f4.0, f6.3, 1x, i3, t55, a1)
          tenline = ' '
...
c     *    '.', evsum(35:36), ' ', evsum(37:37), '.', evsum(38:38)
          read(evsum,817) iyr, imo, idy, ihr, imn, sec, latd, ins, 
     *      rlatm, lond, iew, rlonm, depth, rmag
817       format(2x,5i2,f4.2,i2,a1,f4.2,i3,a1,f4.2,f5.2,f2.1)
          write(tenline(1:59),818) nev, iyr, imo, idy, ihr, imn, sec,
     *    latd, rlatm, ins, lond, rlonm, iew, depth, rmag
818       format(2x,i3,i4,'/',i2.2,'/',i2.2,i3,':',i2.2,':',f4.1,i5,
     *      f5.1,a1,i5,f5.1,a1,f6.1,f5.1)
c         if (tenline(11:11) .eq. ' ') tenline(11:11) = '0'
c         if (tenline(14:14) .eq. ' ') tenline(14:14) = '0'
c         if (tenline(20:20) .eq. ' ') tenline(20:20) = '0'
...
90        format (t82, f3.0, f3.0, f4.0, f6.3, 1x, i3, t132, a1)
        else
c inftyp = 3
          fit = 0.
          iline = iline + 1
...
c         cy = ymarg + rmax1 + yskip*float(jmod(num - 1, ncol))
c	  Mitch change jmod to mod Mac OS X has no jmod
c	  Hope this works
          cy = ymarg + rmax1 + yskip*float(mod(num - 1, ncol))
...
c
          ...
          if (inftyp .eq. 3) then
            write(tenline(58:88), '(2f4.0, f6.0, 2x, 2f4.0, f6.0)') 
     *      str1, da1, sa1, str2, da2, sa2
          else
            write(tenline(60:89), '(2(i6,i4,i5))') 
...
d         call transp      
     ...
     *      iaz1,idip1,iaz2,idip2, fit
            ...
            write(tenline(91:114), '(2(i5,i3),f8.3)') 
     *      iaz1,idip1,iaz2,idip2, fit
          endif
c
...
c plot nodal planes 
c
	  if (darkflt) call newpen(6)
          call plotpl (cx, cy, da1, pi, rad, rmax1, dd1 - 90., prot)
          if (darkflt) call newpen(neppctc)
...
931     iline = iline + 1
     ...
     *    parea, tarea
          ...
          write(tenline(115:131), '(i5, i4, 4x, i4)') 
     *    nint(parea*100.), nint(tarea*100.), nfirst
          write(10, '(a)') tenline(1:131)
	endif
c
