...
c
      print *, 'distmx, fmagmn, ijeff, iunit, minobs, maxobs'
      print *, 'munit, mxqual, mxstat, nkil, nrev, cmpsum'
      print *, distmx, fmagmn, ijeff, iunit, minobs, maxobs
      print *, munit, mxqual, mxstat, nkil, nrev, cmpsum
      wtmax = 0.
      prcntx = 0.
...
55        if(.not. cmpsit) goto 70
          inlast = (k - 1) - nsofar
          print *, 'this event has ', inlast, 'first motions.'
          print *, 'of these ', kwt, ' will be used.'
          if( kwt .lt. minobs .or. kwt .gt. maxobs) then
            k = k - inlast
            nused = nused - 1
          else if(dnstrng(cmpsum) .ne. 'none') then
            write (munit, 13) event
          endif
          nsofar = k - 1
          print *, 'found instruction record for event:'
          print *, event
          print *, 'nsofar = ', nsofar, ' nused = ', nused,
     &    'nsum = ', nsum
...
c
70      continue
        print *, 'read ', k-1, ' first motions from ', nused,
     &  ' events.'
...
84      print *, 'decode error with following line:'
     ...
     *x, f5.2)'
        goto 50
85      prmk(k)(3:3) = fm
        print *, 'prmk(k), dist(k), az(k) ', prmk(k), dist(k), az(k)
        print *, 'ain(k), fm, pres ', ain(k), fm, pres

        print *, 'check prmk (', prmk(k), ') for p or blank'
...
c
d       print *, 'check first motion symbol'
     ...
     &      fm .ne. '-') goto 50
c
d       print *, 'check that disk(k) .le. ', distmx
        if (dist(k) .gt. distmx) then
          xcode(k)(4:4) = 'd'
          errat = .5
        endif
c
d       print *, 'skip if angle of inc is within range ', xainmn, xainmx
        if((ain(k) - xainmn)*(ain(k) - xainmx) .le. 0.) then
          xcode(k)(2:2) = 'i'
          errat = .5
        endif
c
d       print *, 'skip if pres (',pres,') is .gt. presmx'
        if (abs(pres) .gt. presmx) then
          xcode(k)(5:5) = 'r'
          errat = .5
        endif
c
d       print *, 'take care of weight code'
        read (prmk(k), '(3x, i1)') ipwt
d       print *, 'ipwt = ', ipwt
        jwt = ipwt + 1
d       print *, 'skip if jwt (', jwt,') is .ge. 5'
        if (jwt .ge. 5) jwt = 5
        ...
        jjwt(k) = jwt
c
d       print *, take care of + and - first motions
        if (fm .eq. '+') then
...
1002  continue
      if(nrwt .eq. 0) then
d        print *, 'this will not work! all readings have zero weight.'
d        print *, 'name, xcode, prmk, dist, az, ain'
d        do 1010 k = 1, nr
d          print *, stn(k), xcode(k), prmk(k), dist(k), az(k), ain(k)
d1010    continue        
        nr = 0
        return
      endif
