/*Program to determine the best events to use in cross-correlation of 
  ...
  where the .difmax variables specify the distances within which to accept
  event pairs for correlation.   The maxlinks parameter sets an upper cutoff
  on the number of events to be associated to a given event for setting up
  cross-correlations.  This limits the links created in a dense volume of
  earthquakes.  If maxlinks = 0, then no upper limit is placed on the number
  of links.  This may be OK for a small cluster of events, but will be unwise 
  for large datasets.  There is no minimum links parameter; while it is true 
  that there may be isolated events with only one or two links, a decision on 
  whether or not to use such poorly linked events can be postponed until
  HYPODD runs.  The last parameter specifies the output file.

  NOTE! This assumes origins have been time sorted.
*/
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
