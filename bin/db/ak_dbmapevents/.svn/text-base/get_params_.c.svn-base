...
get_params_(filename_in,custom_title,special_files,nsf,istaplt,istnam,
		...
		ititl,ipumps,iflt,icol,itran,ilegnd,lfilename,lcustom,lspecial) 
char 	filename_in[512],custom_title[512],special_files[512];
int	*nsf;
int 	*istaplt,*istnam,*ipdeplt,*ipdepth,*iporid,*icities,*ipipe,*iblue;
int 	*ipmag,*idcirc,*ititl,*ipumps,*iflt,*icol,*itran,*ilegnd;
int	lfilename,lcustom,lspecial;
{
	/* Mitch adds */
	char *c_name;
	/* End Mitch adds */
...
/* Mitch adds */
c_name = newcs (filename_in, 512);
(void) strcpy(filename_in, c_name);
free(c_name);
	filename = (char *) malloc(strlen(filename_in) * sizeof(char));
	(void) strcpy(filename,filename_in);
	TRIM(filename);
	...
	*ilegnd = pfget_int (pf,"ilegnd");

	(void) strcpy(title,pfget_string(pf,"title"));
	PAD(title,(int) lcustom);
	(void) strcpy(custom_title,title);

	tbl = pfget_tbl(pf,"label_files");
	*nsf = maxtbl(tbl);
	(void) strcpy(files,"");
	for(i=0;i<*nsf;i++) {
		(void) strcat(files,gettbl(tbl,i));
		(void) strcat(files,".label ");
	}
	...
	*nsf += num_bp;
	for(i=0;i<num_bp;i++) {
		(void) strcat(files,gettbl(tbl,i));
		(void) strcat(files,".bp ");
	}
	PAD(files,(int) lspecial);
	(void) strncpy(special_files,files,lspecial);
