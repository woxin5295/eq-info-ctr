...
char 	filename_in[512],custom_title[80],special_files[512];
int	*nsf;
...
int	lfilename,lcustom,lspecial;
{
	...
	if(strncmp (filename_in, "dbmap_release", 13 ) == 0) {
		filename_in[13] = '\0';
	}
	...
	if( pfread (filename,&pf) != 0) {
		fprintf(stderr,"get_params: pfread barfed for %s\n",filename);
		exit(1); 
	}
	...
	*nsf += num_bp;
	...
	/* Mitch mod 20060706 crap cludge */
	/* strcpy(special_files,files); */
	strncpy(special_files,files,lspecial);
